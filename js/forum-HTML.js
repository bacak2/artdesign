function CancelNewWpis(project){
    ...
    $("#answer").val("0");
    //$("#NewWpis").val("");
    CKEDITOR.instances['NewWpis'].setData("");
...
function SaveWpis(project){
    //text = $("#NewWpis").val();
    //console.log(CKEDITOR.instances['NewWpis'].getData());
    text = CKEDITOR.instances['NewWpis'].getData().replace(/(\r\n|\n|\r)/gm,"");
    if(window.opener){
...
function GetSzkic(project){
    ...
    CKEDITOR.instances['NewWpis'].setData(res);
    SaveWpis(project);
